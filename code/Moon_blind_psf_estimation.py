...
detector_otf=detector_blur(3,3,si)
otfTurbulent = np.multiply(tele_otf, atmosphere_otf)
tot_otf=np.multiply(otfTurbulent,detector_otf)
output_img = real(ifft2(np.multiply(otfTurbulent, fft2(photon_img))))
sys_otf=np.multiply(tele_otf,detector_otf)
output_img2=output_img[0:si:3,0:si:3]
# Add Poison Noise
# noisy_img = random_noise(real(output_img), mode='poisson')
noisy_img = np.random.poisson(output_img2)

temp=noisy_img[0:1000:1,500]
temp2=-(temp[0:999:1]-temp[1:1000:1])
xx=np.argmax(temp2)
temp3=temp2[0:xx:1]
temp4=(temp3-np.mean(temp3))/np.std(temp3)
corr_vals=np.zeros(20)
for i in range(0,20):                 
    ro = i*.001+.001
    r1 = D/2
    dx = 4*r1/si
    atmosphere_otf = make_long_otf2(r1,dx,si,ro)
    tot_otf=np.multiply(sys_otf,atmosphere_otf)
    tot_psf=fftshift(ifft2(tot_otf))
    samp_psf=tot_psf[0:3000:3,0:3000:3]
    test_vec=samp_psf[500-166:500:1,500]
    temp5=test_vec.real
    temp6=(temp5-np.mean(temp5))/np.std(temp5)
    corr_vals[i]=np.mean(np.multiply(temp4,temp6))

ro_est=np.argmax(corr_vals)/1000+.001
dxx_actual=lamb*.28/(2*ro_est)
